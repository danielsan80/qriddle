...
describe('TrackNav', () => {
  it('renders all step labels', () => {
    render(<TrackNav step={middle.code} onStep={vi.fn()} />);
  ...
  it('hides next button on last step', () => {
    render(<TrackNav step={last.code} onStep={vi.fn()} />);
    expect(screen.queryByRole('button', { name: /next/i })).toBeNull();
  });

  it('calls onStep with next step on next click', async () => {
    const onStep = vi.fn();
    render(<TrackNav step={middle.code} onStep={onStep} />);
    await userEvent.click(screen.getByRole('button', { name: /next/i }));
    expect(onStep).toHaveBeenCalledWith(TRACK_STEPS[3].code);
  });
  ...
  it('calls onStep when clicking a step label', async () => {
    ...
    expect(onStep).toHaveBeenCalledWith(first.code);
  });
});
